...
- **`common/`** — `VantaDotWidgetTheme` holds widget-specific color/spacing constants. `GlanceText` renders text as bitmaps using the custom Doto font (since Glance doesn't support custom fonts natively).
...
`proguard-rules.pro` keeps `CalendarWidgetReceiver`, `CalendarUpdateWorker`, `VantaDotApp`, and all `androidx.glance.**` classes. Any new widget receiver, worker, or Application subclass must be added here.
