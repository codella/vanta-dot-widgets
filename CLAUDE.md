...
- **`timer/data/`** — `TimerState` data class with `displayTime()` and `progress()`. Supports three modes: COUNTDOWN, STOPWATCH, POMODORO. Uses anchor-based timing (`startedAtMs` + `pausedRemainingMs`) for drift-free calculation.
- **`timer/widget/`** — Glance widget implementation. `TimerWidget` uses `SizeMode.Exact` with COMPACT (4×2: time + controls) and FULL (4×4: time + progress bar + presets + controls + pomodoro indicator). Four action callbacks handle start/pause, reset, preset selection, and mode cycling. `TimerWidgetContent` shows urgency-colored time display that shifts white→amber→orange→red as countdown nears zero.
- **`timer/service/`** — `TimerService` foreground service with `Handler`-based 1-second ticking. Updates all widget instances every second while running. Uses `START_STICKY` to recover after process kill. Handles pomodoro auto-cycling (work→break→work).
- **`timer/notification/`** — `TimerNotificationManager` manages two notification channels: `timer_active` (low priority, ongoing, silent) for the running timer, and `timer_completion` (high priority, sound + vibration) for completion alerts.
...
`proguard-rules.pro` keeps `CalendarWidgetReceiver`, `CalendarUpdateWorker`, `VantaDotApp`, `RefreshActionCallback`, `TimerWidgetReceiver`, `TimerService`, timer callbacks, and all `androidx.glance.**` classes. Any new widget receiver, worker, action callback, service, or Application subclass must be added here.
